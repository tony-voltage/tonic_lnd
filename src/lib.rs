...
pub async fn connect_string<A, CP>(
    address: A,
    ...
    CP: AsRef<Path> + Into<PathBuf> + std::fmt::Debug,
{
    ...
    .tls_config(tls::string(cert_contents).await?)
    .map_err(InternalConnectError::TlsConfig)?
    .connect()
    .await
    .map_err(|error| InternalConnectError::Connect {
...
mod tls {
    ...
    pub(crate) async fn string(
        ...
        tls_config
            .dangerous()
            .set_certificate_verifier(std::sync::Arc::new(CertVerifier::string(contents).await?));
        tls_config.set_protocols(&["h2".into()]);
        Ok(tonic::transport::ClientTlsConfig::new().rustls_client_config(tls_config))
    }
    ...
    impl CertVerifier {
        ...
        pub(crate) async fn string(contents: Vec<u8>) -> Result<Self, InternalConnectError> {
            let mut reader = &*contents;

            let certs = try_map_err!(rustls_pemfile::certs(&mut reader), |error| {
                InternalConnectError::ParseCertNoFile { error }
            });
